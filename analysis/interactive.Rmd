...
```{r}
file.copy("output/04-marker-detection.rds", "shiny/integrated/data.rds", overwrite = TRUE)
```
...
# Create iSEE object
iSEE(sce, appTitle = "LPMP", colormap = ecm)
```

Deploy the iSEE application to the shiny server:

```{r}
deployApp(appDir = "shiny/integrated", appName = "LPMP", appTitle = "LPMP", launch.browser = FALSE, forceUpdate = TRUE)
```

Allocate more memory to the iSEE application:

```{r}
configureApp(appName = "LPMP", appDir = "shiny/integrated", size = "xxlarge")
```
