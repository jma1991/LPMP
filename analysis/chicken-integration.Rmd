...
dat$batch[dat$batch == "chicken_st5_1"] <- "chicken_st5" # collapse replicates for plotting

dat$batch[dat$batch == "chicken_st5_2"] <- "chicken_st5" # collapse replicates for plotting

dat$Cluster[dat$batch != "chicken_st5"] <- NA # ignore mouse clusters

ChickenClusterColours <- c(
  "st5_0" = "#c0c0c0", # Epi
  "st5_1" = "#20b2aa", # Meso
  "st5_2" = "#ff6347", # Neural
  "st5_3" = "#ffd700", # NMP
  "st5_4" = "#a9a9a9", # Endo
  "st5_5" = "#add8e6", # NC
  "st5_6" = "#ab6ad5"  # Missing
)

ChickenClusterLabels <- c(
  "st5_0" = "Epi",
  "st5_1" = "Meso",
  "st5_2" = "Neural",
  "st5_3" = "NMP",
...
ggplot(dat, aes(UMAP.1, UMAP.2, colour = Cluster)) + 
  geom_point(data = dat[, c("UMAP.1", "UMAP.2")], colour = "#f2f2f2") + 
  geom_point(size = 2) + 
  scale_colour_manual(values = ChickenClusterColours, labels = ChickenClusterLabels, na.value = "#000000") + 
  labs(x = "UMAP 1", y = "UMAP 2") + 
  facet_wrap(~ batch, labeller = labeller(
    batch = c("chicken_st5" = "St5 (Chicken)",
              "mouse_lpmp_ehf" = "LPMP-EHF (Mouse)",
              "mouse_lpmp_lhf" = "LPMP-LHF (Mouse)"
    )
  )) + 
