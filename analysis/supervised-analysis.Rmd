...
Set row subset using gene identifier:
...
plotHeatmap(
  ...
  exprs_values = "reconstructed",
  center = TRUE,
  colour_columns_by = c("label", "stage"),
...
```{r}
res <- findMarkers(sce, colLabels(sce), block = sce$batch, test.type = "t", pval.type = "any")
```
...
sce <- slingshot(sce, clusterLabels = colLabels(sce), reducedDim = "PCA", start.clus = 2)
```
...
Identify batch with cells greater than required degrees of freedom:
